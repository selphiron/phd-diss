\chapter{Deriving Road Surface Quality from Cycling Trip Data}
...
\begin{itemize}
	...
	\item We describe how we integrate this data processing pipeline in the existing crowdsourcing project SimRa (See \Cref{cha:cyclesense}) which previously only focused on incidents.
...
The following outline details the structure of this chapter:

First, we give background information about the measurement of road surface quality (\Cref{sec:surface_quality_background}).
We then continue with the scenarios and constraints of our approach (\Cref{sec:scenarios_and_constraints}).
We explain our approach and how it is integrated into SimRa (\Cref{sec:data_processing_pipeline}).
After that, we describe two ways in which the road surface quality information can be used (\Cref{sec:using_road_surface_quality_information}).
We follow this up with an evaluation and discussion of our approach (\Cref{sec:evaluation_cyclequality} and \Cref{sec:discussion_cyclequality}).
We conclude this chapter (\Cref{sec:summary_cyclequality}) after giving an overview of alternative approaches (\Cref{sec:related_work_cyclequality}).
...
\begin{figure}%[t]
    %\vspace{-3em}
    \centering
    \subfloat[Smooth cobblestones]{\includegraphics[width=0.47\columnwidth, height=4cm]{fig/cobblestone.jpg}}
    \hfill
    \subfloat[Rough asphalt]{\includegraphics[width=0.47\columnwidth, height=4cm]{fig/asphalt.jpg}}
    \caption{%
...
Second, due to the width of bicycle tires, a single trip on a street segment will only cover a very small percentage of the surface.
...
For this, we consider in the first step each trip individually:
...
\end{figure}
With such an output file, it is very easy to create a simple visualization\footnote{\url{https://simra-project.github.io/surfaceQuality/Berlin.html}}, e.g., with Leaflet\footnote{\url{https://leafletjs.com/}}, as depicted in \Cref{fig:visualization}.
...
\toprule%
Street Name          & Surface       & \ac{gps} Location        & Mean & Median & Std. Dev.\\%
\midrule%
\midrule%
...
\Cref{tab:mismatch} shows two sections, that are -- according to \ac{osm} -- paved with cobblestones and without separate bike lanes, but with very good results, which is confirmed by the PDFs depicted in \Cref{fig:mismatched}.
...
\begin{figure}%[t]
    %\vspace{-3em}
    \centering
    \subfloat[Straßburger Straße]{\includegraphics[width=0.47\columnwidth]{fig/straßburger_str_2.png}}
    \hfill
    \subfloat[\label{fig:goethe}Goethestraße]{\includegraphics[width=0.47\columnwidth]{fig/goethestr22.png}}
    %
    \caption{%
...
\Cref{fig:sidewalk} also shows very likely reasons why these sections have very smooth cycling trip.
...
Considering these circumstances, it is not surprising to see good surface quality values in these two and other similar streets as cyclists are likely to (illegaly) use the sidewalks instead, as shown in~\Cref{fig:goethe}.
...
As discussed in~\Cref{subsec:limitations_of_crowdsourced_data}, crowdsourced data comes with some limitations.
