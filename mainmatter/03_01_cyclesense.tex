...
\vspace*{\fill}
\chapter{Gathering Cycling Trip Data and Detecting Near Miss Incidents}
...
Although we can use data produced by the \ac{imu}, which provides us information about acceleration, orientation and other gravitational forces, to automatically detect these information, we first need to know how the \ac{imu} footprint of an incident looks like.
That is where we rely on the SimRa users, who after each ride add information about the incidents that happened during the trip.
However, a cycling trip can be very long, both in distance and time, which complicates this process for the SimRa users since it can be difficult to remember every incident and their exact location.
To make their task easier, SimRa has two different mechanisms to automatically detect where an incident occurred, so that the user can easier remember the incidents and their specifics to provide them.
...
With their rise, \ac{dnn} has drawn attraction to itself from the field of \ac{tsc}.
...
As shown in figure~\ref{fig:deep_learning_structure}, a Deep Learning approach to \ac{tmc} consists of a classifier (in this case a \ac{mlp}, which is a type of a \ac{dnn}), that maps a time series to a probability distribution of $K$ classes.
...
\textit{Auto Encoders}, such as Stacked Denoising Auto-Encoders~\cite{bengio2013generalized}, \acp{cnn}~\cite{song2020representation}, Deep Belief Networks~\cite{banerjee2019deep}, \acp{cnn}~\cite{rajan2018generative}, split the the unsupervised learning preprocessing
...
%For example, one of the incident hotspots in terms of close passes that we were able to identify in SimRa did not even have a single bicyle trip in the Radmesser data.
...
Now, that we have over 10,000 labeled trips, we started to explore alternative detection methods ranging from machine learning to signals processing.
...
Note, that we store trips and user data per region so that we can analyze (geographic) regions separately (we describe our concept of regions in section~\ref{sec:data}).
